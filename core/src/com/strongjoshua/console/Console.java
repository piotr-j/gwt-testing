...
//		stage.getViewport().setWorldSize(width, height);
	...
	private void execCommand(String command) {
		...
		for(int i = 0; i < size; i++) {
			...
			if(numArgs != paramTypes.length)
				continue;

			try {
				m.invoke(exec, args);
				return;
			} catch(IllegalArgumentException | ReflectionException | ClassCastException e ) {
				String msg = e.getMessage();
				if(msg == null || msg.length() <= 0 || msg.equals("")) {
					msg = "Unknown Error " + e.toString() + " "+e.getCause();
					e.printStackTrace();
				}
